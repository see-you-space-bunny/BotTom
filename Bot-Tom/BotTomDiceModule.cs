...
namespace BotTom
{
	...
	[Option("Class", "The class you want to roll up.")] string? mbClass
	)
		{
			...
			Scum scum = new (name:scvm.Name,background:$"Created by: {context.User.Username}",abilityScores:new int[]{
				scvm.Abilities.Agility,scvm.Abilities.Strength,scvm.Abilities.Toughness,scvm.Abilities.Presence
			},hitPoints:scvm.HitPoints,omens:scvm.Omens.Die);

			var guid = System.Guid.NewGuid().ToString();

			string messageBody = scvm.ToString();
			...
			return context.CreateResponseAsync(messageBody);
		}
	}
}